  ...
  (use-package mu4e
    ...
    (setq mu4e-user-mail-address-list '("sean.michael.farley@gmail.com"
          ...
          '(("maildir:/inbox AND flag:unread AND NOT flag:trashed" "All unread" ?u)
            ...
            ("maildir:/lists macports" "Macports" ?p)
...
#+BEGIN_SRC emacs-lisp
  (use-package prodigy
    ...
    ;; filter function to send password to the openconnect process
    (defun smf/vpn-filter (&rest args)
      (let* ((service (plist-get args :service))
             (output (plist-get args :output))
             (proc (plist-get service :process)))

        (when (and proc (s-matches? "Password" output))
          (let ((pass (concat (smf/auth-source-pass "sfarley@atlassian.com" "atlassian.my.centrify.com") "\rpush\r")))
            (process-send-string proc pass)
            (clear-string pass)
            (process-send-eof proc)))

        (when (s-matches? "Established DTLS connection" output)
          (prodigy-set-status service 'ready))))

    (prodigy-define-service
      :name "Atlassian VPN"
      :command (expand-file-name "~/projects/dotfiles/vpn-connect")
      :args '("vpn.atlassian.com" "-u" "sfarley")
      :tags '(work vpn)
      :on-output 'smf/vpn-filter
      :stop-signal 'kill
      :kill-process-buffer-on-stop t)
...
#+BEGIN_SRC emacs-lisp
  ...
  (use-package anaconda-mode
    ...
    :config
    ;; below, the keymaps need to be set in :config instead of :bind since :bind
    ;; turns on deferred loading in the opposite way (loads when user presses
    ;; those keys)

    ;; By default, anaconda binds =M-,= to finding assignments instead of popping
    ;; the navigation stack (which changed in Emacs 25)
    (define-key anaconda-mode-map (kbd "M-,") nil)

    ;; prefer to use other jump packages
    (define-key anaconda-mode-map (kbd "M-.") nil))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  ...
  (use-package auth-password-store
    ...
    ;; needed to initialize auth-source-pass correctly as emacs is spawning a new
    ;; process
    (add-hook 'async-smtpmail-before-send-hook #'auth-pass-enable)
    :config
    (auth-pass-enable))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  (use-package ein
    :ensure t
    :config
    (setq ein:jupyter-default-server-command "jupyter-2.7"
...
A decent prompt from [[http://www.modernemacs.com/post/custom-eshell/][Modern Emacs]]. We'll start with some macros to help out,
...
Set up eshell with some prompt characters (concrete) and other basic settings,
...
#+BEGIN_SRC emacs-lisp
  (use-package eshell
    ...
    (setq eshell-history-size 32768)
