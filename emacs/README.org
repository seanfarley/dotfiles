...
#+BEGIN_SRC emacs-lisp
  ...
  (setq-default channels '("&bitlbee" "#mercurial" "#bitbucket" "#rw_grim"))
...
#+BEGIN_SRC emacs-lisp
  (use-package mu4e
    ...
    ;; function for launching mu4e along with prodigy service
    (defun smf/mu4e-start-or-switch ()
      "Start the imapnotify process or switch to the mu4e buffer"
      (interactive)
      ;; switch to the scratch buffer so cwd isn't in a project
      (with-current-buffer (get-buffer "*scratch*")
    ...
    (defun smf/disable-email ()
      (interactive)
      (if (get-buffer " *mu4e-main*") ;; ERC already active?
...
#+BEGIN_SRC emacs-lisp
  (use-package org
    ...
    (setq-default org-startup-folded nil
                  ...
                  '((nil :maxlevel . 2)   ; top level headlines in current buffer
                    (("~/Dropbox/org/entertainment.org") :maxlevel . 4)
  ...
  (use-package tiny-menu
    ...
    (add-hook 'org-mode-hook
              (lambda ()
                (define-key org-mode-map "\C-c\C-t" (tiny-menu-run-item "org-tasks"))
                (define-key org-agenda-mode-map "\C-c\C-t" (tiny-menu-run-item "org-agenda-tasks"))

                ))

    (defun smf/org-task-change ()
      (interactive)
      (tiny-menu-run-item "org-tasks"))

    (setq tiny-menu-items
          '(("org-tasks" ("Org Tasks"
                          ((?t "TODO"    (lambda () (interactive) (org-todo "TODO")))
                           (?s "SOMEDAY" (lambda () (interactive) (org-todo "SOMEDAY")))
                           (?d "DONE"    (lambda () (interactive) (org-todo "DONE")))
                           (?c "CANCELED"    (lambda () (interactive) (org-todo "CANCELED"))))))
            ("org-agenda-tasks" ("Org Agenda Tasks"
                          ((?t "TODO"    (lambda () (interactive) (org-agenda-todo "TODO")))
                           (?s "SOMEDAY" (lambda () (interactive) (org-agenda-todo "SOMEDAY")))
...
#+BEGIN_SRC emacs-lisp
  (use-package writeroom-mode
    ...
    ;; helper method to turn on "Do Not Disturb" mode in macos
    (defun smf/do-not-distrub (&optional disable)
      "Toggles the Do Not Disturb button in MacOS Notification
      Center.
  ...
  %s
    ...
    (advice-add 'writeroom--enable :before #'smf/do-not-distrub)
    (advice-add 'writeroom--disable :before (lambda () (smf/do-not-distrub t))))
#+END_SRC
