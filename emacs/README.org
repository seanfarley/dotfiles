...
of the buffer. We'll bind this to =H-M-d=
...
#+begin_src emacs-lisp
  ...
  (global-set-key (kbd "H-M-d") 'smf/delete-to-end-of-buffer)
#+end_src
...
For the few times that I edit a dos script,
...
By default, I hate substatement being indented past their parent line, so we
turn that off. Also, we enable =flycheck-mode=, =eldoc=, and =nlinum-mode=,
...
omnisharp-emacs is a port of the awesome [[https://github.com/nosami/OmniSharpServer][OmniSharp]] server to the Emacs text
editor. It provides IDE-like features for editing files in C# solutions in
Emacs, provided by an OmniSharp server instance that works in the background.

: This requires the Omnisharp Server program

The server must be at least the following version (expect this to be update to
this guide whenever the required version changes):
...
I store the omnisharp-server in a hidden directory,

#+begin_src emacs-lisp
  (setq omnisharp-server-executable-path
    (concat
      (smf/join-dirs smf/dotfiles-dir "omnisharp-server/OmniSharp/bin/Debug")
      "OmniSharp.exe"))
#+end_src

Map some common functions to keys,

#+begin_src emacs-lisp
  (eval-after-load 'omnisharp
    '(progn
       (define-key omnisharp-mode-map (kbd "s-.") 'omnisharp-auto-complete)
       (define-key omnisharp-mode-map (kbd "s-2") 'omnisharp-find-usages)
       (define-key omnisharp-mode-map (kbd "s-3") 'omnisharp-go-to-definition)
       (define-key omnisharp-mode-map (kbd "s-1") 'omnisharp-fix-code-issue-at-point)))
#+end_src

Finally, start omnisharp-mode with csharp-mode,
...
#+begin_src emacs-lisp
  ...
  (defun smf/erc-chat (nick)
    ...
    (unless (memq nick (smf/buffer-users "&bitlbee")) (set-buffer "#mercurial"))
    (erc-cmd-QUERY nick))
...
A lot of examples have offlineimap run every 5 or so minutes. I don't like this
because it causing the mail view to [[https://github.com/djcb/mu/issues/514][pause while reading email]]. So, we'll turn it
off and manually update.

Side note: from practice I've found that running offlineimap with no new
messages to sync takes more than two minutes for 75,000 messages.
...
To the best of my knowledge, mu4e + offlineimap is the only way to sync gmail
labels bidirectionally with emacs. Packages such at [[http://notmuchmail.org][notmuch]] don't actually write
the labels into the email file so reading from multiple clients is broken (by
design / philosophy).

Below is the code I use to achieve a gmail-like experience.

Since I use labels instead of folders, we actually don't want to set the trash
folder like below, so it's commented out but left as an example.
...
The easiest way to work with labels and to main auto-complete is to have a
hard-coded list of labels. Adding or removing labels from here is fairly
trivial.
...
these to apply custom labels and override default behavior.
...
#+begin_src emacs-lisp
  (after-load 'mu4e
      ...
      (fmakunbound 'mu4e-view-mark-for-delete)
      ...
      (fmakunbound 'mu4e-headers-mark-for-delete)
