...
**** Generate ligature codes

To generate the above list of ligatures, Mitsuharu Yamamoto provided me this
awesome bit of lisp,

#+begin_src emacs-lisp
  (defvar fira-code-ligatured-string-list
    '(;; 0.6
      "**" "***" "+++" "--" "---" "?:"
      "/=" "/==" ".=" "^=" "=~" "?=" "||=" "|="
      "<<<" "<=<" "-<<" "-<" ">-" ">>-" ">=>" ">>>"
  ...
  (defun classify (value-to-key values)
    (let ((alist))
      (dolist (value values)
        (let* ((key (funcall value-to-key value))
               (key-values (assoc key alist)))
          (if key-values
              (unless (member value (cdr key-values))
                (setcdr key-values (cons value (cdr key-values))))
            (push (list key value) alist))))
      alist))

  (defun create-comp-table-data (strings)
    (let ((alist (sort (classify 'string-to-char strings)
                       (lambda (x y) (< (car x) (car y))))))
      (setq alist
            (mapcar (lambda (key-values)
                      (cons (car key-values)
                            (sort (classify 'length (cdr key-values))
  ...
  (defun dump (strings)
    (insert (format "(let ((alist '%s))
    (dolist (char-regexp alist)
      (set-char-table-range composition-function-table (car char-regexp)
          `([,(cdr char-regexp) 0 font-shape-gstring]))))"
                    (pp-to-string (create-comp-table-data strings)))))

  ;;(dump fira-code-ligatured-string-list)
#+end_src
...
Also, please rescale images to the size of the window,
    ...
    (if (get-buffer "*irc-freenode*") ;; ERC already active?
        (progn
          ...
          (if (erc-server-process-alive)
              (select-window (get-buffer-window channel)) ;; yes: switch to #channel
...
#+begin_src emacs-lisp
  (smf/add-launcher "e" (lambda ()
                          (interactive)
                          (persp-switch "email")
                          (select-window (or (get-buffer-window " *mu4e-main*")
                                             (get-buffer-window "*mu4e-view*")))))
#+end_src

*** persp-mode

After trying a few different packages for managing emacs windows
(=perspective=, =workgroups2=, =purpose=), it seems =persp-mode= works best
with my setup.
...
#+begin_src emacs-lisp
  ...
  (global-set-key (kbd "H-s") #'persp-switch)
#+end_src
...
#+begin_src emacs-lisp
  (global-set-key [(control tab)] 'persp-next)
  (global-set-key [(control shift tab)] 'persp-prev)
#+end_src
