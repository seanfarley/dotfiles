  ...
  (add-hook 'after-init-hook
            (lambda ()
              (when (fboundp 'magit-find-file-completing-read)
                (defadvice projectile-current-project-files (around smf/use-magit-find-file activate)
...
#+begin_src emacs-lisp
  ...
  (smf/add-launcher "t" 'magit-find-file-completing-read)
#+end_src
...
#+begin_src emacs-lisp
  (evil-leader/set-key
    ...
    "r" 'smf/recentf-ido-find-file
    "w" 'save-buffer
    "x" 'smex
    "y" 'bury-buffer)
#+end_src

Now we automatically copy across everything from =smf/launcher-map= to ensure I
easily retain muscle memory:

#+begin_src emacs-lisp
  (dolist (entry (cdr smf/launcher-map))
    (evil-leader/set-key
...
#+begin_src emacs-lisp
  (defun smf/company-complete-lambda (arg)
...
#+begin_src emacs-lisp
  (after-load 'evil
    (setq
     evil-complete-next-func 'smf/company-complete-lambda
     evil-complete-previous-func 'smf/company-complete-lambda))
#+end_src
...
#+begin_src emacs-lisp
  (after-load 'flycheck
    (add-to-list 'exec-path (concat smf/dotfiles-dir "node_modules/.bin/")))
#+end_src
...
#+begin_src emacs-lisp
  (after-load 'flycheck
    (defun smf/turn-on-flycheck-mode-js2 ()
    ...
    (add-hook 'js2-mode-hook 'smf/turn-on-flycheck-mode-js2))
#+end_src
...
#+begin_src emacs-lisp
  ...
  (add-hook 'go-mode-hook 'smf/set-local-go-path)
#+end_src
...
#+begin_src emacs-lisp
  (after-load 'company-go
    (add-hook 'go-mode-hook 'smf/setup-company-go))
#+end_src
